...
export abstract class BaseResource {
  ...
  protected parseJsonResponse<T>(toolResult: ToolResult): T {
    ...
    if (!jsonContent) {
      throw new SecureLendError('Invalid response from MCP server: missing JSON content', 'mcp_error', toolResult);
    }

    try {
        return jsonContent.type === 'text' ? JSON.parse(jsonContent.text) : JSON.parse((jsonContent as any).resource.text);
    } catch (e) {
        throw new SecureLendError('Invalid response from MCP server: failed to parse JSON content', 'mcp_error', toolResult);
    }
  }
